package rorkh.fakeizen.debugging;
    ...
    public static String getCoreHeader() {
        ...
        try {
            ...
            return "Server Version: " + Bukkit.getServer().getName() + " version " + Bukkit.getServer().getVersion()
                    + "\nActive Plugins (" + (pluginCount - 1) + "): " + pluginlist.substring(0, pluginlist.length() - 2)
                    ...
                    + "\nMode: " + ChatColor.GREEN + "online" + modeSuffix;
        }
        catch (Throwable ex) {
            Debug.echoError(ex);
